    ...
    <div id="portfolio" class="section gray">
      ...
      $i = 1;
			while ($row = mysql_fetch_assoc($rez)) {
				$q = "SELECT * FROM `cards_images` where `card_id`='" . $row["id"] . "' order by (`id`) LIMIT 1";
				$r = mysql_query($q);
				$rw = mysql_fetch_assoc($r); ?>
						  <div class="col l4 m6 s12 gallery-item gallery-expand gallery-filter polygon" order="<?=$i?>" category="<?=$row['category']?>">
